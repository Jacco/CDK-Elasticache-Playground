...
function nextWeekDay(day: Weekday): Weekday {
    const nxt: { [day: string]: Weekday } = {
        'mon': Weekday.TUESDAY,
        'tue': Weekday.WEDNESDAY,
        'wed': Weekday.THURSDAY,
        'thu': Weekday.FRIDAY,
        'fri': Weekday.SATURDAY,
        'sat': Weekday.SUNDAY,
        'sun': Weekday.MONDAY
    }
    return nxt[day]
...
export interface MaintenanceWindowProps {
  /**
   * The day of the week for maintenance to be started. If only
   * this property is specified the window will be from 03:00-05:00
   * in the early moring of that day.
   */
...
export class MaintenanceWindow {
  /**
   * The day of the week for maintenance to be started.
   */
  private readonly startDay: Weekday;
  /**
   ...
   * The day of the week for maintenance to be started.
   */
  private readonly endDay: Weekday;
  /**
   * The hour of the day (from 00-24) for maintenance to be started.
   */
  ...
  constructor(props: MaintenanceWindowProps) {
    ...
    let endDay: Weekday = props.startDay;
    if (endMinute > 59) {
        endMinute -= 59;
        endHour += 1;
    }
  ...
  public toString(): string {
    ...
    return `${this.startDay.valueOf()}:${this.startHour}:${this.startMinute}-${this.endDay.valueOf()}:${this.endHour}:${this.endMinute}`;
  }
